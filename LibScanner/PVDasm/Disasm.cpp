		 ...
		 ) 
	{
		switch(Op)
		{
			...
			{                   
				...
				if(RegRepeat>1) 
				{
					strcpy_s(Disasm->Opcode,30,"66:");
					strcpy_s(Disasm->Remarks,256,"Prefix DataSize:");
					Disasm->OpcodeSize=1;
					Disasm->PrefixSize=0;
					(*Index)-=RegRepeat;
					return;
			...
			case 0x67: // Addr prefix, change default Reg size, (EDI->DI) and more!
			{
				...
				if(AddrRepeat>1) 
				{
					strcpy_s(Disasm->Opcode,30,"67:");
					strcpy_s(Disasm->Remarks,256,"Prefix AddrSize:");
					Disasm->OpcodeSize=1;
					Disasm->PrefixSize=0;
					(*Index)-=AddrRepeat;
					return;
			...
			case 0x0F0: // LockPrefix, Add bus lock menemonic opcode in front of every menemonic
			{
				...
				if(LockRepeat>1) 
				{
					strcpy_s(Disasm->Assembly,256,"");
					strcpy_s(Disasm->Opcode,30,"F0:");
					strcpy_s(Disasm->Remarks,256,"Prefix LOCK:");
					Disasm->OpcodeSize=1;
					Disasm->PrefixSize=0;
					(*Index)-=LockRepeat;
					return;
			...
			case 0x64: case 0x65: // Segment Prefixes
			{
				...
				{                    
					  ...
					  )
					{
                        ...
                        strcpy_s(Disasm->Opcode,30,menemonic);
...
// 				if (Op==0x9A)
// 				{
// 					DWORD CallOff=dwMem-BASEADDRES;
// 					PBYTE pCallData=pCodeBase+CallOff;
// 					CallHandle(pCallData,Disasm);
// 				}
			}
			else
			{
...
// 				if (Op==0x9A)
// 				{
// 					DWORD CallOff=w_mem-BASEADDRES;
// 					PBYTE pCallData=pCodeBase+CallOff;
// 					CallHandle(pCallData,Disasm);
// 				}
			}
		...
		case 0xE8:case 0xE9: // CALL/JMP XXXX/XXXXXXXX
		{
			...
			if(!RegPrefix)
			{
				...
				if (Op==0xE8)
				{
					DWORD CallOff=dwMem-BaseAddress;
					PBYTE pCallData=pCodeBase+CallOff;
					CallHandle(pCallData,Disasm);
				}
			}
			else 
			{
				...
				if (Op==0xE8)
				{
					DWORD CallOff=CallAddress-BaseAddress;
					PBYTE pCallData=pCodeBase+CallOff;
					CallHandle(pCallData,Disasm);
				}
			}
...
void ShowDecoded(DISASSEMBLY* pDisasm,FILE* pfile)
{
	// Printed all information regarding a 
	// Decoded instruction

    fprintf_s(pfile,"%08X: %-30s  %s /%d ; %s",pDisasm->Address,
			                           pDisasm->Opcode,
									   pDisasm->Assembly,
									   pDisasm->OpcodeSize+pDisasm->PrefixSize,
									   pDisasm->Remarks
          );
}
...
void FlushDecoded(DISASSEMBLY *Disasm)
{
    ...
    strcpy_s(Disasm->Assembly,256,""); // Clear menemonic
    strcpy_s(Disasm->Remarks,256,"");  // Clear commets
    strcpy_s(Disasm->Opcode,30,"");   // Clear opcodes linear
    Disasm->OpcodeSize=1;        // Smallest opcode size
	Disasm->PrefixSize=0;        // No Prefixes
}

void PVDasm(const PBYTE pCodeData,const DWORD CodeDatSize,DWORD baseAddress,FILE* pFile)
{
	...
	pCodeBase=pCodeData;
    Linear=(char*)pCodeData;   // Points to the address of array to decode.
	BaseAddress=baseAddress;
    Disasm.Address=BaseAddress; // Common Entry Point (usually default..)
    FlushDecoded(&Disasm);     // reset all content
    
    for(Index=0;Index<CodeDatSize;Index++)
    {
		// Decode instruction
